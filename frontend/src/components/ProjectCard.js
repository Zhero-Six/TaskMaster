...
const ProjectCard = ({ id, title, description, creator = {}, status }) => {
  ...
  const creatorId = typeof creator === 'object' ? creator.id : null;
  const creatorName = typeof creator === 'object' ? creator.username : creator || 'Unknown';
  ...
  return (
    <div style={{ backgroundColor: 'white', padding: '16px', borderRadius: '8px', boxShadow: '0 1px 2px rgba(0, 0, 0, 0.1)' }}>
      ...
      <p style={{ color: 'var(--gray-600)' }}>Creator: {creatorName}</p>
